...
) callconv(.{ .x86_64_win = .{} }) std.os.windows.LRESULT;
...
) callconv(.{ .x86_64_win = .{} }) ?std.os.windows.HWND;

pub extern "user32" fn RegisterClassExA(
    *const WNDCLASSEXA,
) callconv(.{ .x86_64_win = .{} }) std.os.windows.ATOM;

pub extern "user32" fn SetWindowLongPtrA(
    hWnd: std.os.windows.HWND,
    nIndex: i32,
    dwNewLong: std.os.windows.LONG_PTR,
) callconv(.{ .x86_64_win = .{} }) std.os.windows.LONG_PTR;
...
) callconv(.{ .x86_64_win = .{} }) std.os.windows.BOOL;
...
pub extern "user32" fn GetDC(
    hWnd: ?std.os.windows.HWND,
) callconv(.{ .x86_64_win = .{} }) ?std.os.windows.HDC;

pub extern "user32" fn ReleaseDC(
    hWnd: ?std.os.windows.HWND,
    hDC: std.os.windows.HDC,
) callconv(.{ .x86_64_win = .{} }) i32;
...
pub extern "user32" fn CreatePopupMenu() callconv(.{ .x86_64_win = .{} }) std.os.windows.HMENU;
...
) callconv(.{ .x86_64_win = .{} }) std.os.windows.HICON;
...
) callconv(.{ .x86_64_win = .{} }) std.os.windows.UINT;
